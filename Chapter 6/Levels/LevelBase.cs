...
namespace MonoGameRPG
{
    public abstract class LevelBase
    {
        ...
        public virtual bool IsSolid(Vector2 point)
        {
            ...
            List<MapTile> tiles = Tiles.Where(t => t.Data.IsSolid && target.Intersects(new Rectangle((int)t.Location.X, (int)t.Location.Y, tileSize.X, tileSize.Y))).ToList();

            if (tiles != null && tiles.Count > 0)
                return true;

            tiles = OverlayTiles.Where(t => t.Data.IsSolid && target.Intersects(new Rectangle((int)t.Location.X, (int)t.Location.Y, tileSize.X, tileSize.Y))).ToList();
        ...
        public virtual TileData IsExitTo(Vector2 point)
        {
            ...
            MapTile tile = Tiles.FirstOrDefault(t => !string.IsNullOrEmpty(t.Data.ExitTo) && target.Intersects(new Rectangle((int)t.Location.X, (int)t.Location.Y, tileSize.X, tileSize.Y)));

            if (tile != null)
                return tile.Data;

            tile = OverlayTiles.FirstOrDefault(t => !string.IsNullOrEmpty(t.Data.ExitTo) && target.Intersects(new Rectangle((int)t.Location.X, (int)t.Location.Y, tileSize.X, tileSize.Y)));
