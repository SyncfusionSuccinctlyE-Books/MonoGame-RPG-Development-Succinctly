...
namespace MonoGameRPG
{
    ...
    public delegate void EntityKilledEventHandler(EntityKilledEventArgs e);
    ...
    public class GameObject
    {
        ...
        public void Initialize(string fileName)
        {
            ...
            //GameSprite = new Sprite(spriteSheet, new Point(32, 40), new Point(16, 20));
            GameSprite = new Sprite(spriteSheet, new Point(spriteSheet.Width, spriteSheet.Height / 2), new Point(spriteSheet.Width / 2, spriteSheet.Height / 4));
        ...
        public static List<EntityGameObject> LoadNPCs()
        {
            ...
            string data = File.ReadAllText(@"Content\Data\Levels\" + Global.CurrentLevelName + @"\npcs.json");
            ...
            else
            {
                ...
                if (input.IsNewMouseClick(MouseButtons.Left))
                {
                    if (Global.RectClicked(GameSprite.Position, GameSprite.Size, input.CurrentMouseState[0].X, input.CurrentMouseState[0].Y))
                    {
                        ...
                        if (EntityType == EntityType.NPC && ((Entity)Entity).HasConversation(out id))
                        {
                            Click(((Entity)Entity).ID, id);
                        }
                        else if (EntityType == EntityType.Monster || EntityType == EntityType.Creature)
                        {
                            //temp code until combat is in place
                            //kill entity
                            EntityKilled(new EntityKilledEventArgs() { ID = ((Entity)this.Entity).ID, Tag = ((Entity)this.Entity).Name });
