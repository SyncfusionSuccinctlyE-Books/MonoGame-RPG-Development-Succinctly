    ...
    public class Conversation
    {
        ...
        public void CheckPreFunction()
        {
            if(curNode.NodeFunctionType == FunctionType.PreFunction)
            {
                ...
                if(curNode.Responses[0].NodeFunctionType == FunctionType.PreFunction)
                {
                    curNode = curNode.Responses[0];
                    CheckPreFunction();
                }
            }
        }
        ...
        public void SelectResponse(int index)
        {
            ...
            if(curNode.Responses[index].NodeFunctionType == FunctionType.PostFunction && curNode.Responses != null)
            {
                ...
                if (curNode.Responses[index].Responses != null)
                {
                    if ((bool)ret)
                    {
                        curNode = curNode.Responses[index].Responses.Find(c => c.NodeCaseType == CaseType.CaseTrue).Responses[0];
                    }
                    else
                    {
                        curNode = curNode.Responses[index].Responses.Find(c => c.NodeCaseType == CaseType.CaseFalse).Responses[0];
                    }
                }
                else
                {
