    ...
    public class GameplayScreen : GameScreen
    {
        ...
        public override void Activate(bool instancePreserved)
        {
            if (!instancePreserved)
            {
                ...
                int w = ScreenManager.GraphicsDevice.Viewport.Height;

                if (playerInventory == null)
                {
                    playerInventory = new PlayerInventory(_content);


                    Weapon sword = new Weapon(_content.Load<Texture2D>("Sprites/Inventory/Sword"), new Point(32, 32))
                    {
                        Value = 55,
                        Weight = 5,
                        Damage = "D6",
                    ...
                    Armor boots = new Armor(_content.Load<Texture2D>("Sprites/Inventory/boots"), new Point(32, 32))
                    {
                        Value = 5,
                        Weight = 2,
                        ArmorValue = 3,
                    ...
                    Armor helm = new Armor(_content.Load<Texture2D>("Sprites/Inventory/helm"), new Point(32, 32))
                    {
                        Value = 15,
                        Weight = 2,
                        ArmorValue = 10,
                    ...
                    Weapon bow = new Weapon(_content.Load<Texture2D>("Sprites/Inventory/bow"), new Point(32, 32))
                    {
                        Value = 20,
                        Weight = 1,
                        Damage = "D4",
                        Name = "Bow",
                        Description = "A light bow, useles without arrows",
                        EquipableLocation = EquipableLocation.TwoHanded,
                    };

                    Ammunition arrows = new Ammunition(_content.Load<Texture2D>("Sprites/Inventory/arrow"), new Point(32, 32),"Bow","Long Bow")
                    {
                        Name = "Arrow",
                        Quantity = 12,
                        Description = "You will need a Bow to use these.",
                        Weight = 1,
                    };

                    ConsumableItem healingPotion = new ConsumableItem(_content.Load<Texture2D>("Sprites/Inventory/healingPotion"), new Point(32, 32))
                    {
                        Value = 100,
                        Weight = 1,
                        Quantity = 3,
                    ...
                    Armor shield = new Armor(_content.Load<Texture2D>("Sprites/Inventory/shield"), new Point(32, 32))
                    {
                        Value = 25,
                        Weight = 5,
                        ArmorValue = 50,
                        Name = "Shield",
                        Description = "A beautiful kite shield, should protect you from nasty blows.",
                        EquipableLocation = EquipableLocation.Hand,
                    };

                    Key key1 = new Key(_content.Load<Texture2D>("Sprites/Inventory/key"), new Point(32, 32))
                    {
                    ...
                    //{
                    //    Weapon tst = new Weapon(_content.Load<Texture2D>("Sprites/Inventory/Sword"), new Point(32, 32))
                    //    {
                    //        Value = 55,
                    //        Weight = 5,
                    //        Damage = "D6",
        ...
        {
            // We don't want this as our player inventory needs to stay across scenes..
            //_content.Unload();
        }
        ...
        public override void HandleInput(GameTime gameTime, InputState input)
        {
            ...
            else
            {
                if (!playerInventory.IsShowing)
                {
                    ...
                    {
                        playerInventory.Pickup(currentLevel.PickupItem());
                    }
                }

                if (input.IsNewKeyPress(Keys.I, ControllingPlayer, out player))
                {
                    playerAvatar.animationPlayer.StartClip("Idle");
                    playerInventory.IsShowing = !playerInventory.IsShowing;
                }

                playerInventory.HandleInput(gameTime, ControllingPlayer, input);

                if (playerInventory.DroppedItem != null)
                {
                    // Place item in current map.
                    currentLevel.AddItem(playerInventory.DroppedItem);
                    playerInventory.DroppedItem = null;
                }
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            if (playerInventory.IsShowing)
            {
                playerInventory.Position = new Vector2(8, 20);
